#include "TimeManagerWindow.h"

TimeManagerWindow::TimeManagerWindow(const WindowType type, const std::string& name) :EditorWindow(type, name)
{

}

TimeManagerWindow::~TimeManagerWindow()
{
}

void TimeManagerWindow::DrawWindow()
{
    if (!IsEnabled()) return;
...
void TimeManagerWindow::DrawPlaybackControls()
{
    ...
    // Botón Play
    bool isPlaying = !ModuleTimeManager::GetInstance()->IsPlaying();
    if (!isPlaying) {
    ...
    // Botón Pause
    bool isPaused = ModuleTimeManager::GetInstance()->IsPlaying() &&
...
void TimeManagerWindow::DrawTimeInfo()
{
    ImGui::Spacing();
    ImGui::Separator();
